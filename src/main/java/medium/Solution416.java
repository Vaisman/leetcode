...
public class Solution416 {
  public boolean canPartition(int[] nums) {
    ...
    // Thus in this problem, by using the sum value as the index of DP array, we transfer the
    // problem
    ...
    // current DP value
    // if we take the nums[i], then we need to find whether the (new_sum = j - nums[i]) can be
    // constructed.
  ...
  public boolean canPartition2(int[] nums) {
    ...
    System.out.println(Integer.toBinaryString(state.intValue()));
    for (int num : nums) {

         state = state.or(state.shiftLeft(num));
        System.out.println(num + "  " +Integer.toBinaryString(state.intValue()));
    }
    System.out.println(Integer.toBinaryString(state.intValue()));
  ...
  @Test
  public void test() {
    assertEquals(canPartition2(new int[] {1, 5, 10, 1, 5}), true);
  }
}
